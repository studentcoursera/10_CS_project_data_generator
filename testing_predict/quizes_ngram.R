...
## Author		: Ambika J
## Date			: May-2016
## Notes		: This file is to get results for the quizes (using the ngram predict)
...
for (i in 1:length(quiz2)) {
    print(paste(quiz2[i], predict_ngram_next_word(quiz2[i]), sep=":"))
}

correct_count2 <- 0
for (i in 1:length(quiz2)) {
    ordr_tmp_results <- prediction_ngram_result_set(quiz2[i])
...
quiz3options <- c("give|die|eat|sleep",
                  ...
                  "morning|month|weekend|decade",
                  "hunger|sleepiness|happiness|stress",
                  "look|minute|picture|walk",
                  "incident|matter|case|account",
                  "arm|toe|hand|finger",
                  "center|top|middle|side",
...
for (i in 1:length(quiz3)) {
    print(paste(i, "> ",quiz3[i], predict_ngram_next_word(quiz3[i]), sep=":"))
}

correct_count3 <- 0
for (i in 1:length(quiz3)) {
    ordr_tmp_results <- prediction_ngram_result_set(quiz3[i])
